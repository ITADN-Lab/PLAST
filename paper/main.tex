...
\paragraph{Architecture robustness.} The result is not a small-network artifact. The gap holds across ResNet-18 widths $32/64/128$: Muon$+$RP $77.2/80.5/79.2$ vs.\ best AdamO $69.3/70.7/71.0$, i.e.\ $\Delta{=}{+}7.9/{+}9.8/{+}8.2$ ($n{=}3$). The advantage is thus stable in the $+8$ to $+10$ range as capacity grows $16\times$ in parameters---not a small-width artifact.
...
\includegraphics[width=0.78\linewidth]{fig_rank_acc.png}
\caption{\textbf{Effective rank predicts plasticity.} Per-task accuracy vs.\ penultimate effective rank, one point per (method, seed, task)---$250$ points (CIFAR-100), $r{=}0.73$; within-method average $+0.44$. As \S\ref{sec:scope} notes, this is driven by separating the collapsed optimizer (Adam) from the rest---rank does not separate the strong optimizers from each other.}
\label{fig:rankacc}
\end{figure}
\begin{figure}[h]\centering
